...
public class WendigoStateB : WendigoBaseState
{
    private float danger;

    public override void EnterState(WendigoScript wendigoScript)
    {
        Debug.Log("Wendigo is on the offensive");
        Debug.Log("current state is " + wendigoScript.currentState);
        danger = wendigoScript.wendigoDangerLevel;
    }
    public override void UpdaterState(WendigoScript wendigoScript)
    {
        ...
        if (!wendigoScript.attack || (Input.GetKey(KeyCode.LeftShift) && Input.GetKey(KeyCode.W) && Input.GetKeyDown(KeyCode.I)))
        {
            wendigoScript.SwitchState(wendigoScript.aState);
        }
    }
